    ...
    ensures AllEventsValid(events + [new_event], num_frames, min_consecutive)
  {
              ...
              (events + [new_event])[j].start_frame
    {
      ...
      if j < |events| {
        ...
        // j == |events|: extended[j] == new_event
        assert extended[i] == events[i];
        assert extended[j] == new_event;
        assert i < |events|;
        // Prove: events[i].end_frame < new_event.start_frame
        // via: events[i].end_frame < events[|events|-1].end_frame+1
        //      <= events[|events|-1].end_frame < new_event.start_frame
        var last := |events| - 1;
        if i == last {
          assert events[last].end_frame < new_event.start_frame;
        } else {
          // i < last: use AreNonOverlapping to get chain
          assert events[i].end_frame < events[last].start_frame
              by { assert AreNonOverlapping(events); }
          // events[last].start_frame <= events[last].end_frame
          // follows from IsValidEvent which requires start_frame <= end_frame
          assert events[last].end_frame < new_event.start_frame;
        }
      }
    }
