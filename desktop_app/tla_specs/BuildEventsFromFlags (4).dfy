...
module BuildEventsFromFlags {
  ...
  {
    // Вспомогательная лемма: для любого i < |events|,
    // events[i].end_frame < new_event.start_frame
    forall i | 0 <= i < |events|
      ensures events[i].end_frame < new_event.start_frame
    {
      var last := |events| - 1;
      if i == last {
        // Прямо из предусловия
      } else {
        // i < last: из AreNonOverlapping следует
        // events[i].end_frame < events[last].start_frame
        // из IsValidEvent: events[last].start_frame <= events[last].end_frame
        // из предусловия: events[last].end_frame < new_event.start_frame
              ...
              (events + [new_event])[j].start_frame
    {
      ...
      if j < |events| {
        ...
        // Уже доказано выше что events[i].end_frame < new_event.start_frame
      }
    }
  }
